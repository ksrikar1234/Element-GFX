...
class EntityManager: details::forbid_copies {
 ...
 public:
  ...
  // Move operator
  inline EntityManager& operator=(EntityManager &&other)
  {
    component_managers_ = std::move(other.component_managers_);
    component_masks_ = std::move(other.component_masks_);
    entity_versions_ = std::move(other.entity_versions_);
    next_free_indexes_ = std::move(other.next_free_indexes_);
    index_to_component_mask = std::move(other.index_to_component_mask);
    component_mask_to_index_accessor_ = std::move(other.component_mask_to_index_accessor_);
    block_count_ = other.block_count_;
    count_ = other.count_;

    other.component_managers_.clear();
    other.component_masks_.clear();
    other.entity_versions_.clear();
    other.next_free_indexes_.clear();
    other.index_to_component_mask.clear();
    other.component_mask_to_index_accessor_.clear();
...
class SystemManager: details::forbid_copies {
 public:
  ...
  /// Pop a system from the system manager
  template<typename S>
  inline S* pop();
  ...
  template<typename S>
  inline void activate_only();
  
  inline void reactivate_all();

  inline uint32_t count();

 private:
  std::vector<System *> systems_;
  std::vector<System *> inactive_systems_;
...
S& SystemManager::add(Args &&... args) {
  ...
  if(details::system_index<S>() >= systems_.size())
  { 
    systems_.resize(details::system_index<S>() + 1);
    inactive_systems_.push_back(nullptr);
    std::vector<System *> temp;
    temp.resize(systems_.size());
    for(auto& system : temp) system = nullptr;
    for(int i = 0; i < inactive_systems_.size(); ++i)
        temp[i] = inactive_systems_[i];
  }
...
S& SystemManager::get() {
  ...
  return *static_cast<S *>(systems_[details::system_index<S>()]);
}
...
void SystemManager::remove() {
  ECS_ASSERT_IS_SYSTEM(S);
  ECS_ASSERT(exists<S>(), "System does not exist");
  S* poped_system = pop<S>();
  if(pop<S>() != nullptr) delete poped_system;
}

template<typename S>
S* SystemManager::pop() {
  ECS_ASSERT_IS_SYSTEM(S);
  System* poped_system = systems_[details::system_index<S>()];
  if(poped_system == nullptr)
  poped_system = inactive_systems_[details::system_index<S>()];

  systems_[details::system_index<S>()] = nullptr;
  inactive_systems_[details::system_index<S>()] = nullptr;
  ...
  return static_cast<S *>(poped_system);
}

template<typename S>
bool SystemManager::activate() {
  ECS_ASSERT_IS_SYSTEM(S);
  bool system_already_exists = (details::system_index<S>() < inactive_systems_.size() && inactive_systems_[details::system_index<S>()] != nullptr);
  if(system_already_exists){
    systems_[details::system_index<S>()] = inactive_systems_[details::system_index<S>()];
    inactive_systems_[details::system_index<S>()] = nullptr; 
    return true;
  }
  else
...
void SystemManager::deactivate() {
  ECS_ASSERT_IS_SYSTEM(S);
  ECS_ASSERT(exists<S>(), "System does not exist");
  S* poped_system = systems_[details::system_index<S>()];
  systems_[details::system_index<S>()] = nullptr;

  inactive_systems_[details::system_index<S>()] = poped_system;
}


template<typename S>
void SystemManager::activate_only()
{
  ECS_ASSERT_IS_SYSTEM(S);
  ECS_ASSERT(systems_.size() == inactive_systems_.size(), "Systems and inactive systems size mismatch");
  for(int i = 0; i < systems_.size(); i++)
  {   
    if(systems_[i] != nullptr)
    {
      inactive_systems_[i] = systems_[i];
      systems_[i] = nullptr;
    }
  }
...
void SystemManager::reactivate_all()
{
  std::swap(systems_, inactive_systems_);
}
...
{
  std::vector<std::exception> exceptions_list;
  exceptions_list.resize(0);
  ...
  for (auto index : order_)
  {
    ...
    {
      exceptions_list.push_back(e);
      std::cerr << "Error initializing system: " << e.what() << "\n";
    ...
    if(exceptions_list.size() != 0) throw exceptions_list;
  }
}
...
inline bool SystemManager::has() {
  ...
  (inactive_systems_.size() > details::system_index<S>() && inactive_systems_[details::system_index<S>()] != nullptr);
}

inline uint32_t SystemManager::count() {
  return order_.size();
}
