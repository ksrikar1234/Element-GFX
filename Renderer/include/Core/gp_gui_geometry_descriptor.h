...
/// @brief GeometryRenderPipelineDescriptor 
...
public:
    struct LIB_API PrimitiveSetInstance {
        ...
        explicit PrimitiveSetInstance(const std::string& _InstanceName,  GLenum _PrimitiveType, const std::shared_ptr<std::vector<float>>& geometry_pos_array);
       ...
       /// @brief Get the Instance Name
        ...
        size_t get_pickable_entities_count() const
        {
            ...
            switch(pickScheme)
            {
                ...
                case PICK_BY_VERTEX: count = get_num_unique_positions(); break;
        ...
        {
            normals.reset(); colors.reset(); indices.reset();
            
           *positions = std::vector<float>(0);
        ...
        { *positions = std::vector<float>(0);     dirtyFlags |= DIRTY_POSITIONS; }
        ...
        const std::vector<float> &positions_vector()
        {
            return *positions;
        }
        ...
        /// @warning Do not const cast !!!
        const std::vector<float> &normals_vector()
        {
            return *normals;
        }
        ...
        /// @warning Do not const cast !!!
        const std::vector<uint8_t> &colors_vector()
        {
            return *colors;
        }
        ...
        /// @warning Do not const cast !!!
        const std::vector<uint32_t> &indices_vector()
        {
            return *indices;
        }
      ...
      public:
        ...
        struct Color
        {
            ...
            std::array<float, 4> get_color_float_array()
            {
                return get_color();
            }
    ...
    std::shared_ptr<std::vector<float>> positions; /* Common Across all primitives */
    ...
    {   
        ...
        currentPrimitiveSet->pick_color_reservation.end  = currentPrimitiveSet->pick_color_reservation.start + reserve - 1;   
...
*/
    ...
    /// @param src , 
    /// @param dst
    __INLINE__ void copy_attrib_array(const std::string& src, const std::string& dst, PrimitiveSetInstance::VertexAttribArrayType type);
    ...
    /// @param src 
    /// @param dst 
    __INLINE__ void share_attrib_array(const std::string& src, const std::string& dst, PrimitiveSetInstance::VertexAttribArrayType type);
    ...
    /// @param src , 
    /// @param dst
    __INLINE__ void copy_attrib_array_with_all(const std::string& src, PrimitiveSetInstance::VertexAttribArrayType type);
    ...
    /// @param src
    /// @param dst
    __INLINE__ void share_attrib_array_with_all(const std::string &src, PrimitiveSetInstance::VertexAttribArrayType type);
    ...
    /// @brief    Set precise pick scheme of the current primitive set
    /// @param GLenum scheme
    __INLINE__ void set_precise_point_pickable(bool flag);
